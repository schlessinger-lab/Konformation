...
def HelixAxis( Input ):
  ...
  for idx, Seq in enumerate(Pre_Coords):
    if Seq is None:
      print('  # Helix Warning: Missing residue: {0}\t{1}'.format(pdb_name, idx+1))
      return None
...
#  TfReg1 = TransformVectorPoints(Reg1, phi, psi)
  ...
  if Center[4] is not None:
    ...
    if Center[6] is not None:
      print('  # Helix Warning: No CG, use "CB" for Ax-Cg vector: '+pdb_name)
      cb_vec = np.array( Center[6] - Reg2[ArrayCent(len(Coords))] )
      sc_vec = np.array( Center[5] - Reg2[ArrayCent(len(Coords))] )
      cg_vec = cb_vec
      ...
      print('  # Helix Warning: No CG for Ax-Cg vector: '+pdb_name)


  if curve is None:
    print('  # Helix Warning: Helix residues < 5, skip "curv": '+pdb_name)

  return [ pdb_name, res_id, axis, cg_nom, cg_vec, sc_vec, 
